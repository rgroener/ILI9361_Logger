...
uint32_t bme280_readPressure(void){
    ...
    uint32_t adc_P = bme280_read3Byte(BME280_REGISTER_PRESSUREDATA);
    ...
    return p/2.56;//2.56 um 2 Nachkommastellen zu haben
}
...
int32_t bme280_readAltitude(float seaLevel){
    // seaLevel at hPa (mBar), equation from datasheet BMP180, page 16
    
    int32_t atmospheric = bme280_readPressure() / 100.0F;
